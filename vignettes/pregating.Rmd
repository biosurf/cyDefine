...
After the classification, we identify unassigned using a per-celltype mahalanobis distance threshold with a 2.5 MAD factor and using a random forest probability threshold of 60% certainty. This is a rather tight gating, but that is reasonable when trying to capture debris, doublets, and dead cells.
...
classified_live$query <- identify_unassigned(
  classified_live,
  markers = markers,
  identify_type = c("maha", "probability"),
...
The bottom row in the heatmap shows which celltypes were identified as unassigned in the raw data, compared to how the manual pregating and prediction on live cells classifies those cells.
